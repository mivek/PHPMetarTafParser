...
final class MainVisibilityNauticalMilesCommand implements Command
{
    private const MAIN_VISIBILITY_SM_REGEX = "/^((\d)*(\s)?(\d\/\d)?)SM$/";
    ...
    {
        preg_match_all(self::MAIN_VISIBILITY_SM_REGEX, $code, $matches);
        if($container->getVisibility() == null) {
            $container->setVisibility(new Visibility());
        }
        $container->getVisibility()->setMainVisibility($matches[1][0], 'SM');
    }
